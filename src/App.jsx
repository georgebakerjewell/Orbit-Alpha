...
// ─────────────────────────────────────────────
// EARNINGS CALENDAR — update this every Sunday
// ─────────────────────────────────────────────
const EARNINGS = [
...
const LAUNCHES = [
  ...
  { date:"Est. May 2026", mission:"Blue Origin – New Glenn / ASTS BlueBird Block 2", status:"TBD", impact:"+12.4% avg", ticker:"ASTS" },
...
export default function App() {
  ...
  useEffect(()=>{
    ...
    const handleMouseLeave = (e) => {
      if(e.clientY <= 0 && !popupDismissed && !popupSubmitted) {
  ...
  // Subscriber count — update this weekly
  ...
  const fetchQuote = async (ticker, range="7d") => {
    try {
      ...
      // Use the second-to-last close as previous day close
      const closePrices = quotes?.close?.filter(Boolean) || [];
      const prevClose = closePrices.length >= 2 
        ? closePrices[closePrices.length - 2]  // yesterday's close
        : meta.chartPreviousClose || meta.previousClose;
      const changePct = meta.regularMarketChangePercent !== undefined
        ? meta.regularMarketChangePercent
        : prevClose ? ((price - prevClose) / prevClose) * 100 : 0;
      const change = meta.regularMarketChange !== undefined
  ...
  const fetchAllPrices = async () => {
    try {
      const tickers = ALL_TICKERS.slice(0,30);
      for(let i=0; i<tickers.length; i+=5) {
        const batch = tickers.slice(i, i+5);
        // Fetch all three ranges simultaneously per batch
        const results = await Promise.allSettled(batch.map(async t => {
          ...
          return {
            ...base,
            spark7d: r7.value?.sparkline || [],
            spark3d: r3.value?.sparkline || [],
            spark1d: r1.value?.sparkline || [],
          };
        }));
        const updates = {};
        results.forEach(r => {
          if(r.status==="fulfilled" && r.value) {
            ...
            updates[d.ticker] = {
              ...
              spark3d: d.spark3d,
              spark1d: d.spark1d,
              liveSparkline: d.spark7d,
            };
          }
        });
        if(Object.keys(updates).length > 0) {
          setLiveStocks(prev => prev.map(s =>
            updates[s.ticker] ? {...s, ...updates[s.ticker]} : s
          ));
  ...
  useEffect(()=>{
      ...
      try {
        ...
        if(data.result && data.result.length > 0) {
          ...
          const mapped = data.result
            .filter(l => {
              // Keep if sort_date is in the future or unknown
              if(!l.sort_date) return true;
              return l.sort_date > now;
            })
            .map(l => {
            const ticker = 
              l.provider?.slug?.includes('rocket-lab') ? 'RKLB' :
              l.name?.includes('BlueBird') || l.missions?.[0]?.name?.includes('BlueBird') ? 'ASTS' :
              l.name?.includes('Intuitive') || l.missions?.[0]?.name?.includes('IM-') ? 'LUNR' :
              null;
    ...
    fetchLaunches();
    const interval = setInterval(fetchLaunches, 3600000); // refresh hourly
    return ()=>clearInterval(interval);
  },[]);

  // News feed — pull from both RSS and Yahoo Finance
    ...
    const fetchNews = async () => {
      try {
        const [rssRes, yahooRes] = await Promise.allSettled([
          ...
          fetch('https://orbit-alpha-api.vercel.app/api/yahoonews').then(r=>r.json()),
        ]);
  ...
  // Skeleton loading — hide after 8 seconds regardless
  ...
  const filtered = (() => {
    ...
    if(sortCol) {
      arr = [...arr].sort((a,b)=>{
        let av = sortCol==="price" ? a.price :
                 sortCol==="changePct" ? a.changePct :
                 sortCol==="mktCap" ? (parseFloat(a.mktCap?.replace(/[^0-9.]/g,""))||0) : 0;
        let bv = sortCol==="price" ? b.price :
                 sortCol==="changePct" ? b.changePct :
                 sortCol==="mktCap" ? (parseFloat(b.mktCap?.replace(/[^0-9.]/g,""))||0) : 0;
  ...
  return (
    <div style={{minHeight:"100vh",background:"#04060e",color:"#dde1ec",fontFamily:"'DM Mono',monospace",fontSize:13,position:"relative",overflowX:"hidden"}}>
      ...
      {showExitPopup&&(
        <div style={{position:"fixed",bottom:24,right:24,zIndex:9999,animation:"fu 0.3s ease",maxWidth:340,width:"calc(100% - 48px)"}}>
          <div style={{background:"#0d1220",border:"1px solid rgba(0,255,136,0.2)",borderRadius:8,padding:"24px",boxShadow:"0 8px 40px rgba(0,0,0,0.6)"}}>
            ...
            <div style={{fontFamily:"'Syne',sans-serif",fontSize:17,fontWeight:800,color:"#fff",lineHeight:1.2,marginBottom:8}}>
              The only weekly covering every space stock.
            </div>
            <div style={{fontSize:11,color:"#aab8c2",lineHeight:1.6,marginBottom:16}}>
              Macro overview · Broker target changes · One stock deep dive. Every Sunday morning.
            </div>
            <div style={{display:"flex",gap:6,marginBottom:12}}>
              <input
                ...
                style={{flex:1,background:"rgba(255,255,255,0.06)",border:"1px solid rgba(255,255,255,0.12)",color:"#fff",padding:"9px 12px",borderRadius:4,fontSize:12,fontFamily:"'DM Mono',monospace",outline:"none",minWidth:0}}
              />
              <button onClick={submitPopup} style={{background:"#00ff88",color:"#04060e",border:"none",padding:"9px 14px",borderRadius:4,fontSize:11,fontWeight:700,fontFamily:"'DM Mono',monospace",cursor:"pointer",whiteSpace:"nowrap",flexShrink:0}}>
                Join →
              </button>
            </div>
      ...
      <style>{`
        ...
        @keyframes tw{0%{opacity:0.12}100%{opacity:0.22}}
        ...
        @keyframes pg{0%,100%{box-shadow:0 0 0 0 rgba(0,255,136,0.2)}50%{box-shadow:0 0 0 10px rgba(0,255,136,0)}}
        ...
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        .nl{cursor:pointer;font-size:11px;letter-spacing:0.1em;text-transform:uppercase;color:#555;transition:color 0.2s;padding:10px 0}.nl:hover{color:#bbb}
      ...
      <div style={{position:"relative",zIndex:2}}>
        ...
        <nav style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"14px 20px",borderBottom:"1px solid rgba(255,255,255,0.05)",position:"relative"}}>
          ...
          <div style={{display:"flex",gap:10,alignItems:"center",flexShrink:0}}>
            <button className="desk-only" onClick={()=>window.open(BEEHIIV_URL,"_blank")} style={{background:"rgba(0,255,136,0.07)",border:"1px solid rgba(0,255,136,0.2)",color:"#00ff88",padding:"7px 15px",borderRadius:4,fontSize:10,letterSpacing:"0.1em",textTransform:"uppercase",fontFamily:"'DM Mono',monospace",cursor:"pointer"}}>
              Subscribe Free
            </button>
            <button className="desk-only" onClick={()=>{
              if(navigator.share){navigator.share({title:"Orbit Alpha",text:"Free space stocks dashboard — live prices, launches, earnings and weekly newsletter",url:"https://orbitalpha.cloud"});}
              else{navigator.clipboard.writeText("https://orbitalpha.cloud").then(()=>alert("Link copied!"));}
            }} style={{background:"none",border:"1px solid rgba(255,255,255,0.1)",color:"#ccd0d8",padding:"7px 12px",borderRadius:4,fontSize:10,letterSpacing:"0.06em",fontFamily:"'DM Mono',monospace",cursor:"pointer"}}>
              Share ↗
            </button>
            <button onClick={()=>setMenuOpen(!menuOpen)} style={{background:"none",border:"1px solid rgba(255,255,255,0.1)",color:"#ccd0d8",padding:"6px 10px",borderRadius:4,cursor:"pointer",fontSize:16,display:"none"}} className="mob-menu-btn">
              {menuOpen?"✕":"☰"}
            </button>
          </div>
        </nav>
        ...
        {menuOpen&&(
          <div style={{background:"#070a14",borderBottom:"1px solid rgba(255,255,255,0.06)",padding:"16px 20px",display:"flex",flexDirection:"column",gap:4}}>
            ...
            <button onClick={()=>{window.open(BEEHIIV_URL,"_blank");setMenuOpen(false);}} style={{background:"#00ff88",color:"#04060e",border:"none",padding:"11px",borderRadius:4,fontSize:11,fontWeight:700,letterSpacing:"0.1em",textTransform:"uppercase",fontFamily:"'DM Mono',monospace",cursor:"pointer",marginTop:8}}>
              Subscribe Free →
            </button>
          </div>
        )}
        ...
        {/* Ticker and banners — home and dashboard only */}
        {(page==="home"||page==="dashboard")&&<Ticker stocks={liveStocks}/>}

        {/* LATEST ISSUE BANNER — home only */}
        {page==="home"&&(
        <div onClick={()=>window.open("https://orbit-alpha.beehiiv.com/p/orbit-alpha-issue-3","_blank")} style={{background:"rgba(126,184,255,0.06)",borderBottom:"1px solid rgba(126,184,255,0.12)",padding:"8px 20px",textAlign:"center",cursor:"pointer",transition:"background 0.2s"}}
          ...
          <span style={{fontSize:11,color:"#7eb8ff",letterSpacing:"0.04em"}}>
            📬 <strong>Issue #3 is live</strong> — SpaceX goes retail · LUNR's $1B moment · NYSE Space Summit
            <span style={{marginLeft:10,opacity:0.6}}>Read now →</span>
          </span>
        </div>
        )}
        {/* DEMO/LIVE BANNER — only show on home and dashboard */}
          ...
          <div style={{animation:"fu 0.5s ease"}}>
            ...
            <section style={{padding:"40px 20px 28px",textAlign:"center",maxWidth:680,margin:"0 auto"}}>
              ...
              <div style={{display:"flex",gap:10,justifyContent:"center",flexWrap:"wrap"}}>
                <button onClick={()=>go("dashboard","stocks")} style={{background:"#00ff88",color:"#04060e",border:"none",padding:"11px 24px",borderRadius:4,fontSize:11,fontWeight:700,letterSpacing:"0.1em",textTransform:"uppercase",fontFamily:"'DM Mono',monospace",cursor:"pointer"}}>
                  View Dashboard →
                </button>
                <button onClick={()=>window.open(BEEHIIV_URL,"_blank")} style={{background:"none",border:"1px solid rgba(255,255,255,0.15)",color:"#aab8c2",padding:"11px 24px",borderRadius:4,fontSize:11,letterSpacing:"0.1em",textTransform:"uppercase",fontFamily:"'DM Mono',monospace",cursor:"pointer"}}>
                  Subscribe Free →
                </button>
              </div>
            </section>

            {/* ── ORBIT DASHBOARD SECTION ── */}
            <section style={{margin:"0 20px 16px",borderRadius:10,border:"1px solid rgba(0,255,136,0.2)",background:"rgba(0,255,136,0.02)",padding:"24px",maxWidth:920,marginLeft:"auto",marginRight:"auto"}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16,flexWrap:"wrap",gap:10}}>
                <div>
                  <div style={{fontFamily:"'Syne',sans-serif",fontSize:18,fontWeight:800,color:"#fff",letterSpacing:"-0.02em",marginBottom:2}}>
                    ORBIT <span style={{color:"#00ff88"}}>DASHBOARD</span>
                  </div>
                  <p style={{fontSize:11,color:"#aab8c2",lineHeight:1.5}}>Live prices, launches, earnings and news — updated automatically.</p>
                </div>
                <button onClick={()=>go("dashboard","stocks")} style={{background:"none",border:"1px solid rgba(0,255,136,0.3)",color:"#00ff88",padding:"8px 16px",borderRadius:4,fontSize:10,fontWeight:700,letterSpacing:"0.1em",textTransform:"uppercase",fontFamily:"'DM Mono',monospace",cursor:"pointer",whiteSpace:"nowrap",flexShrink:0}}>
                  View Dashboard →
                </button>
              </div>
              <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8}}>
                {[
                  {e:"📈",t:"Live Prices",d:"Real-time quotes, 7D charts and market cap. Updated every 60 seconds."},
            ...
            <section style={{margin:"0 20px 16px",borderRadius:10,border:"1px solid rgba(255,150,50,0.2)",background:"rgba(255,150,50,0.02)",padding:"24px",maxWidth:920,marginLeft:"auto",marginRight:"auto"}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16,flexWrap:"wrap",gap:10}}>
                <div>
                  <div style={{fontFamily:"'Syne',sans-serif",fontSize:18,fontWeight:800,color:"#fff",letterSpacing:"-0.02em",marginBottom:2}}>
                    ORBIT <span style={{color:"#ff9632"}}>NEWS</span>
                  </div>
                  <p style={{fontSize:11,color:"#aab8c2",lineHeight:1.5}}>Live space stock news from 30+ sources — updated every 5 minutes.</p>
                </div>
                <button onClick={()=>go("news")} style={{background:"none",border:"1px solid rgba(255,150,50,0.3)",color:"#ff9632",padding:"8px 16px",borderRadius:4,fontSize:10,fontWeight:700,letterSpacing:"0.1em",textTransform:"uppercase",fontFamily:"'DM Mono',monospace",cursor:"pointer",whiteSpace:"nowrap",flexShrink:0}}>
                  View News →
                </button>
              </div>
              ...
              {!newsLoading&&newsItems.slice(0,4).map((item,i)=>(
                <div key={i} onClick={()=>window.open(item.link,"_blank")}
                  className="hov" style={{padding:"10px 0",borderBottom:"1px solid rgba(255,255,255,0.04)",cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"center",gap:12}}>
                  <div style={{flex:1,minWidth:0}}>
                    ...
                    <div style={{display:"flex",gap:8,alignItems:"center"}}>
                      <span style={{fontSize:9,padding:"1px 6px",borderRadius:3,
                        background:item.source==="SpaceNews"?"rgba(0,255,136,0.08)":item.source==="NASA"?"rgba(126,184,255,0.08)":"rgba(255,204,0,0.08)",
                        color:item.source==="SpaceNews"?"#00ff88":item.source==="NASA"?"#7eb8ff":"#ffcc00"
                      }}>{item.source}</span>
            ...
            <section style={{margin:"0 20px 40px",borderRadius:10,border:"1px solid rgba(126,184,255,0.2)",background:"rgba(126,184,255,0.02)",padding:"24px",maxWidth:920,marginLeft:"auto",marginRight:"auto"}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16,flexWrap:"wrap",gap:10}}>
                <div>
                  <div style={{fontFamily:"'Syne',sans-serif",fontSize:18,fontWeight:800,color:"#fff",letterSpacing:"-0.02em",marginBottom:2}}>
                    ORBIT <span style={{color:"#7eb8ff"}}>NEWSLETTER</span>
                  </div>
                ...
                <button onClick={()=>go("newsletter")} style={{background:"none",border:"1px solid rgba(126,184,255,0.3)",color:"#7eb8ff",padding:"8px 16px",borderRadius:4,fontSize:10,fontWeight:700,letterSpacing:"0.1em",textTransform:"uppercase",fontFamily:"'DM Mono',monospace",cursor:"pointer",whiteSpace:"nowrap",flexShrink:0}}>
                  View Newsletter →
                </button>
              </div>
              <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8}}>
            ...
            <footer style={{padding:"24px 20px",borderTop:"1px solid rgba(255,255,255,0.04)",display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:12}}>
              ...
              <div style={{display:"flex",gap:16,fontSize:10,color:"#aab8c2",flexWrap:"wrap"}}>
                <span onClick={()=>go("newsletter")} style={{cursor:"pointer",transition:"color 0.2s"}} className="hov">Newsletter</span>
                <span onClick={()=>go("news")} style={{cursor:"pointer",transition:"color 0.2s"}} className="hov">News</span>
                <span onClick={()=>go("about")} style={{cursor:"pointer",transition:"color 0.2s"}} className="hov">About</span>
        ...
        {page==="dashboard"&&(
          <div style={{animation:"fu 0.3s ease"}}>
            ...
            <div style={{padding:"16px 20px 40px"}}>
              ...
              {tab==="stocks"&&(
                <div>
                  {/* Top 5 gainers highlight */}
                  {(()=>{
                    const gainers = [...liveStocks]
                      .filter(s=>typeof s.changePct==="number")
                      .sort((a,b)=>b.changePct-a.changePct)
                      .slice(0,5);
                    if(gainers.length===0) return null;
                    return (
                      <div style={{marginBottom:16}}>
                        <div style={{fontSize:9,color:"#aab8c2",letterSpacing:"0.12em",textTransform:"uppercase",marginBottom:8}}>
                          Top Gainers Today {isLive?"· Live":"· Demo"}
                        </div>
                        <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:5}}>
                          {gainers.map((s,i)=>(
                            <div key={s.ticker}
                              ...
                              className="hov"
                              style={{
                                background:`rgba(0,255,136,${0.04+((5-i)/5)*0.12})`,
                    ...
                    return (
                      <div style={{marginBottom:14}}>
                        ...
                        {[
                          ...
                          <div key={label} onClick={()=>setExpandedTicker(expandedTicker===s.ticker?null:s.ticker)}
                            className="hov" style={{border:`1px solid ${c}22`,borderRadius:6,padding:"10px 14px",background:`${c}08`,cursor:"pointer"}}>
                  ...
                  {/* Search + filters + watchlist toggle */}
                  ...
                  <div className="desk-only" style={{overflowX:"auto"}}>
                    <div style={{minWidth:"auto",width:"100%"}}>
                      ...
                      {!loading && filtered.map(s=>(
                        <div key={s.ticker}>
                          <div
                            className={`hov ${flashMap[s.ticker]==="up"?"flash-up":flashMap[s.ticker]==="down"?"flash-down":""}`}
                            onClick={()=>{
                              setExpandedTicker(expandedTicker===s.ticker?null:s.ticker);
                              if(expandedTicker!==s.ticker) fetchStockNews(s.ticker);
                            }}
                            ...
                            <div style={{display:"flex",alignItems:"center",gap:4}}>
                              <span onClick={e=>{e.stopPropagation();toggleWatch(s.ticker);}} style={{fontSize:12,color:watchlist.includes(s.ticker)?"#ffcc00":"#556",cursor:"pointer",marginRight:2,lineHeight:1}}>
                          ...
                          {expandedTicker===s.ticker&&(
                            <div style={{background:"rgba(0,255,136,0.02)",border:"1px solid rgba(0,255,136,0.1)",borderRadius:6,margin:"0 0 4px",padding:"14px 16px",animation:"fu 0.2s ease"}}>
                              <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:16,marginBottom:10}}>
                                <div>
                                  <div style={{fontSize:9,color:"#aab8c2",letterSpacing:"0.1em",textTransform:"uppercase",marginBottom:4}}>Current Price</div>
                                  <div style={{fontSize:16,color:"#fff",fontWeight:500}}>${s.price.toFixed(2)}</div>
                                </div>
                                <div>
                                  <div style={{fontSize:9,color:"#aab8c2",letterSpacing:"0.1em",textTransform:"uppercase",marginBottom:4}}>1D Change</div>
                                  <div style={{fontSize:16,color:s.changePct>=0?"#00ff88":"#ff4466",fontWeight:500}}>{s.changePct>=0?"+":""}{s.changePct.toFixed(2)}%</div>
                                </div>
                                <div>
                                  <div style={{fontSize:9,color:"#aab8c2",letterSpacing:"0.1em",textTransform:"uppercase",marginBottom:4}}>Mkt Cap</div>
                                  <div style={{fontSize:16,color:"#fff",fontWeight:500}}>{s.mktCap||"—"}</div>
                                </div>
                              </div>
                  ...
                  <div style={{display:"none"}} className="mob-cards">
                    ...
                    <div style={{display:"flex",gap:8,marginBottom:10,alignItems:"center"}}>
                      <button onClick={()=>setShowWatchlistOnly(w=>!w)} style={{background:showWatchlistOnly?"rgba(255,204,0,0.1)":"transparent",border:`1px solid ${showWatchlistOnly?"rgba(255,204,0,0.4)":"rgba(255,255,255,0.08)"}`,color:showWatchlistOnly?"#ffcc00":"#aab8c2",padding:"7px 14px",borderRadius:4,fontSize:10,fontFamily:"'DM Mono',monospace",cursor:"pointer",letterSpacing:"0.08em"}}>
                        ★ {showWatchlistOnly?"Watchlist Only":"Watchlist"}
                      </button>
                    ...
                    {filtered.map(s=>(
                      <div key={s.ticker} style={{border:"1px solid rgba(255,255,255,0.07)",borderRadius:8,marginBottom:8,background:"rgba(255,255,255,0.01)",overflow:"hidden"}}>
                        ...
                        <div onClick={()=>setExpandedTicker(expandedTicker===s.ticker?null:s.ticker)}
                          className={`${flashMap[s.ticker]==="up"?"flash-up":flashMap[s.ticker]==="down"?"flash-down":""}`}
                          style={{padding:"12px 14px",cursor:"pointer"}}>
                          {/* Top row: ticker + name + % change + star */}
                          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
                            <div style={{display:"flex",alignItems:"center",gap:6}}>
                              <span
                                onClick={e=>{e.stopPropagation();toggleWatch(s.ticker);}}
                                style={{fontSize:16,color:watchlist.includes(s.ticker)?"#ffcc00":"#556",cursor:"pointer",lineHeight:1,flexShrink:0}}>
                                {watchlist.includes(s.ticker)?"★":"☆"}
                              </span>
                            ...
                            <span style={{fontSize:11,fontWeight:600,padding:"2px 8px",borderRadius:4,background:s.changePct>=0?"rgba(0,255,136,0.12)":"rgba(255,68,102,0.12)",color:s.changePct>=0?"#00ff88":"#ff4466"}}>
                              {s.changePct>=0?"+":""}{s.changePct.toFixed(1)}%
                            </span>
                          </div>
                          {/* Price + sentiment */}
                          ...
                          {/* Tap hint */}
                          <div style={{textAlign:"center",marginTop:6,fontSize:9,color:"#ccd0d8"}}>
                            {expandedTicker===s.ticker?"▲ tap to close":"▼ tap for more"}
                          </div>
                        </div>
                        ...
                        {expandedTicker===s.ticker&&(
                          <div style={{borderTop:"1px solid rgba(0,255,136,0.1)",padding:"12px 14px",background:"rgba(0,255,136,0.02)",animation:"fu 0.2s ease"}}>
                            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:12}}>
                              <div>
                                <div style={{fontSize:9,color:"#aab8c2",textTransform:"uppercase",letterSpacing:"0.08em",marginBottom:3}}>Current Price</div>
                                <div style={{fontSize:16,color:"#fff",fontWeight:500}}>${s.price.toFixed(2)}</div>
                              </div>
                              <div>
                                <div style={{fontSize:9,color:"#aab8c2",textTransform:"uppercase",letterSpacing:"0.08em",marginBottom:3}}>1D Change</div>
                                <div style={{fontSize:16,color:s.changePct>=0?"#00ff88":"#ff4466",fontWeight:500}}>{s.changePct>=0?"+":""}{s.changePct.toFixed(2)}%</div>
                              </div>
                              <div>
                                <div style={{fontSize:9,color:"#aab8c2",textTransform:"uppercase",letterSpacing:"0.08em",marginBottom:3}}>Mkt Cap</div>
                                <div style={{fontSize:14,color:"#fff",fontWeight:500}}>{s.mktCap||"—"}</div>
                              </div>
                            </div>
                            <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
                              <button onClick={()=>window.open(`https://finance.yahoo.com/quote/${s.ticker}`,"_blank")} style={{flex:1,background:"none",border:"1px solid rgba(126,184,255,0.2)",color:"#7eb8ff",padding:"8px",borderRadius:4,fontSize:10,fontFamily:"'DM Mono',monospace",cursor:"pointer"}}>
                                Yahoo Finance →
                              </button>
                              <button onClick={()=>window.open(BEEHIIV_URL,"_blank")} style={{flex:1,background:"none",border:"1px solid rgba(0,255,136,0.2)",color:"#00ff88",padding:"8px",borderRadius:4,fontSize:10,fontFamily:"'DM Mono',monospace",cursor:"pointer"}}>
                                {s.ticker} in this week's issue →
                              </button>
                            </div>
                            {/* Latest news */}
                            ...
                            {expandedTicker===s.ticker&&!stockNews[s.ticker]&&(
                              <div style={{marginTop:8,fontSize:10,color:"#aab8c2"}}>Loading news...</div>
                            )}
                          </div>
                        )}
                      </div>
              ...
              {tab==="sentiment"&&(
                <div>
                  <div className="mob-grid2" style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10,marginBottom:16}}>
                    {[{l:"Most Bullish",t:"RKLB",v:"78%",c:"#00ff88"},{l:"Highest Volume",t:"ASTS",v:"2,103",c:"#7eb8ff"},{l:"Volume Spike",t:"ASTS",v:"+340%",c:"#ffcc00"},{l:"Most Bearish",t:"SPCE",v:"29%",c:"#ff4466"}].map((c,i)=>(
                      <div key={i} style={{border:"1px solid rgba(255,255,255,0.07)",borderRadius:8,padding:"12px 8px",textAlign:"center",background:"rgba(255,255,255,0.01)"}}>
                        <div style={{fontSize:9,color:"#ccd0d8",letterSpacing:"0.08em",textTransform:"uppercase",marginBottom:6}}>{c.l}</div>
                        <div style={{fontSize:18,fontWeight:500,color:c.c,marginBottom:3}}>{c.v}</div>
                        <div style={{fontSize:11,color:"#aab8c2"}}>{c.t}</div>
                      </div>
                    ))}
                  </div>
                  <div className="mob-grid1" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
                    {STOCKS.slice(0,14).map(s=>(
                      <div key={s.ticker} style={{border:"1px solid rgba(255,255,255,0.06)",borderRadius:8,padding:"12px"}}>
                        <div style={{display:"flex",justifyContent:"space-between",marginBottom:7}}>
                          <div><span style={{fontSize:12,fontWeight:500,color:"#ddd"}}>{s.ticker}</span><span style={{fontSize:10,color:"#ccd0d8",marginLeft:6}}>{s.name}</span></div>
                          <span style={{fontSize:11,color:s.changePct>=0?"#00ff88":"#ff4466"}}>{s.changePct>=0?"+":""}{s.changePct.toFixed(1)}%</span>
                        </div>
                        <SentBar value={s.sentiment}/>
                        <div style={{display:"flex",gap:10,marginTop:7,flexWrap:"wrap"}}>
                          <span style={{fontSize:10,color:"#ccd0d8"}}>MENTIONS <span style={{color:"#ccd0d8"}}>{s.mentions.toLocaleString()}</span></span>
                        </div>
                      </div>
                    ))}
                  </div>
                  ...
                  {EARNINGS.map((e,i)=>(
                    <div key={i} style={{
                      ...
                      <div style={{fontSize:11,color:"#aab8c2",lineHeight:1.5}}>
                        <span style={{color:"#aab8c2",marginRight:6,letterSpacing:"0.08em",fontSize:9,textTransform:"uppercase"}}>Watch:</span>
                        {e.watch}
                      </div>
                    </div>
                  ))}
        ...
        {page==="news"&&(
          <div style={{animation:"fu 0.3s ease",maxWidth:800,margin:"0 auto",padding:"32px 20px 60px"}}>
            ...
            <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:20}}>
              {["All","RKLB","ASTS","LUNR","PL","BKSY","RDW","MNTS","SPCE","KRMN","SATL","KULR","TSAT","GSAT","VSAT","MDA","SPIR","DXYZ","LMT","FLY","OKLO","BA","NOC","RTX","SpaceX","Blue Origin","Relativity","Vast","ispace","NASA","ESA","ISRO","Space Force"].map(co=>(
                <span key={co} onClick={()=>setNewsCompany(co==="All"?"":co)}
                  className="stg" style={{color:newsCompany===(co==="All"?"":co)?"#ff9632":"#ccd0d8",borderColor:newsCompany===(co==="All"?"":co)?"rgba(255,150,50,0.3)":"rgba(255,255,255,0.2)",background:newsCompany===(co==="All"?"":co)?"rgba(255,150,50,0.05)":"transparent",fontSize:9}}>
                  {co}
                </span>
              ))}
            </div>
            ...
            {!newsLoading&&(()=>{
              const COMPANY_KEYWORDS = {
                ...
                ASTS:['AST SpaceMobile','ASTS','BlueBird'],
                LUNR:['Intuitive Machines','LUNR','lunar lander'],
                PL:['Planet Labs','Planet Labs PBC'],
                SPCE:['Virgin Galactic','SPCE'],
                RDW:['Redwire','RDW'],
                SpaceX:['SpaceX','Starship','Falcon','Starlink'],
                NASA:['NASA'],
                'Blue Origin':['Blue Origin','New Glenn'],
              };
              return newsItems
                ...
                <div key={i} onClick={()=>window.open(item.link,"_blank")}
                  className="hov" style={{
                    ...
                    cursor:"pointer",
                    animation:`fu 0.3s ease ${i*0.02}s both`,
                    borderRadius: item.highlight ? 6 : 0,
                    border: item.highlight ? "1px solid rgba(255,204,0,0.15)" : "none",
                    borderBottom: item.highlight ? "1px solid rgba(255,204,0,0.15)" : "1px solid rgba(255,255,255,0.05)",
                    background: item.highlight ? "rgba(255,204,0,0.03)" : "transparent",
                    marginBottom: item.highlight ? 8 : 0,
                  }}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:12,marginBottom:8}}>
                    <div style={{display:"flex",gap:6,alignItems:"center",flexWrap:"wrap"}}>
                      <span style={{fontSize:9,padding:"2px 8px",borderRadius:3,flexShrink:0,
                        background:item.source==="SpaceNews"?"rgba(0,255,136,0.08)":item.source==="NASA"?"rgba(126,184,255,0.08)":item.source==="Space.com"?"rgba(255,255,255,0.04)":"rgba(255,204,0,0.08)",
                        color:item.source==="SpaceNews"?"#00ff88":item.source==="NASA"?"#7eb8ff":item.source==="Space.com"?"#888":"#ffcc00"
                      }}>{item.source}</span>
                    ...
                    <span style={{fontSize:10,color:"#aab8c2",flexShrink:0}}>
        ...
        {page==="newsletter"&&(
          <div style={{animation:"fu 0.3s ease",maxWidth:800,margin:"0 auto",padding:"32px 20px 60px"}}>
            <div style={{marginBottom:28}}>
              ...
              <button onClick={()=>window.open(BEEHIIV_URL,"_blank")} style={{background:"#7eb8ff",color:"#04060e",border:"none",padding:"10px 22px",borderRadius:4,fontSize:11,fontWeight:700,letterSpacing:"0.1em",textTransform:"uppercase",fontFamily:"'DM Mono',monospace",cursor:"pointer"}}>
                Join {SUBSCRIBER_COUNT}+ Free →
              </button>
            </div>
            ...
            {[
              ...
              <div key={i} className="hov" onClick={()=>issue.live&&window.open(issue.url,"_blank")}
                style={{border:"1px solid rgba(255,255,255,0.06)",borderRadius:8,padding:"20px",marginBottom:10,background:"rgba(255,255,255,0.01)",cursor:issue.live?"pointer":"default",opacity:issue.live?1:0.5,position:"relative"}}>
        ...
        {page==="about"&&(
          <div style={{animation:"fu 0.3s ease",maxWidth:640,margin:"0 auto",padding:"32px 20px 60px"}}>
            <div style={{fontFamily:"'Syne',sans-serif",fontSize:28,fontWeight:800,color:"#fff",marginBottom:6}}>
              ABOUT <span style={{color:"#00ff88"}}>ORBIT ALPHA</span>
            </div>
            <div style={{height:1,background:"rgba(255,255,255,0.06)",margin:"20px 0"}}/>
            <p style={{fontSize:13,color:"#aab8c2",lineHeight:1.8,marginBottom:16}}>
              Orbit Alpha is a free dashboard and weekly newsletter built for retail investors who follow space equities. It covers every publicly traded space stock, ETF and private company — with live prices, launch catalysts, broker target changes and weekly deep dives.
            </p>
            <p style={{fontSize:13,color:"#aab8c2",lineHeight:1.8,marginBottom:16}}>
              The newsletter goes out every Sunday morning and covers three things: a macro overview of the week in space stocks, a broker pulse showing all analyst rating and price target changes, and one stock of the week — bull case, bear case, key catalysts, honest view on valuation.
            </p>
            <p style={{fontSize:13,color:"#aab8c2",lineHeight:1.8,marginBottom:32}}>
              Everything is free. No paywall. No signup required to use the dashboard.
            </p>
            <div style={{height:1,background:"rgba(255,255,255,0.06)",marginBottom:24}}/>
            <div style={{fontSize:10,color:"#aab8c2",letterSpacing:"0.12em",textTransform:"uppercase",marginBottom:12}}>Get in touch</div>
            <a href="mailto:OrbitAlphaApp@proton.me" style={{color:"#00ff88",fontSize:13,textDecoration:"none",display:"inline-flex",alignItems:"center",gap:6}}>
              📬 OrbitAlphaApp@proton.me
            </a>
            <div style={{marginTop:24}}>
              <button onClick={()=>window.open(BEEHIIV_URL,"_blank")} style={{background:"#00ff88",color:"#04060e",border:"none",padding:"10px 22px",borderRadius:4,fontSize:11,fontWeight:700,letterSpacing:"0.1em",textTransform:"uppercase",fontFamily:"'DM Mono',monospace",cursor:"pointer"}}>
